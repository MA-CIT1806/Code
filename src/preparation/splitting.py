...
from src.preparation.datasets import AnomalySubset, MultiAnomalyDataset
...
class LeaveOneGroupOutWrapper:
    ...
    def __init__(self, dataset):
        ...
        if isinstance(dataset, MultiAnomalyDataset):
            # make sure all datasets have same number of classes
            if len(set([d.num_classes for d in dataset.datasets])) != 1:
                raise ValueError("datasets have varying number of classes.")

        self.logo = LeaveOneGroupOut()
        self.splits = list(self.logo.split(
            dataset.get_data(), y=None, groups=dataset.get_groups()))
        self.split_iter = iter(self.splits)

    def get_split(self, index=-1, data_transform_steps={}):
        all_but_one_group_indices, left_out_group_indices = self._get_split_indices(
            index)
        ...
        self.dataset.set_transform(
            all_but_one_group_indices, data_transform_steps=data_transform_steps)
...
class TransferLearningWrapper:
    ...
    def get_split(self, index=-1, data_transform_steps={}, left_out_group="cassandra"):
        ...
        print("Train and validation indices: {} \n Test indices: {}".format(
            all_but_one_group_indices, left_out_group_indices))

        # preprocess all data based on training data
        self.dataset.set_transform(
            list(range(len(data_list))), data_transform_steps=data_transform_steps)
