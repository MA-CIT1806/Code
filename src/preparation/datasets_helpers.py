...
def transform_time_series_data(data_list, window_width=10, sliding_window=10, init_fn=None, extraction_target="window", **kwargs):
    ...
    for data in data_list:
        ...
        for idx in range(len(data)):
            ...
            indices = range(0, (arr.size(1) - window_width) +
                            1, sliding_window)
            ...
            for i in range(0, len(indices)):
                if i < len(indices) - 1:
                    ...
                    # change in time wont be that big
                    temp_data_list[i].next_possible_nodes = temp_data_list[i].x
            ...
            if extraction_target == "window":
                ...
                bs = math.floor(
                    len(temp_data_list) / 3) if len(temp_data_list) > 200 else len(temp_data_list)
    ...
    if offline and not online:
        ...
        for i in range(len(x_train_list)):
            ...
            if proba >= np.random.random_sample():
                for dist in dists:
                    noise = dist.sample(x_train_list[i].shape).to(
                        x_train_list[i].device)
                    if cat:
    ...
    if online and not offline:
        ...
        for label in batch.unique():
            if proba >= np.random.random_sample():
                ...
                for dist in dists:
                    noise = dist.sample(
                        (indices.sum(), x_train.size(1))).to(x_train.device)
...
class RandomBatchSampler(torch.utils.data.sampler.Sampler):
    ...
    def __iter__(self):
        ...
        t_res = torch.arange((n // self.batch_size) *
                             self.batch_size).reshape(-1, self.batch_size)
